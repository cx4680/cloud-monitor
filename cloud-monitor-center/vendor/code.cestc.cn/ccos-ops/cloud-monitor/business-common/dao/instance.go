...
func (dao *InstanceDao) SelectInstanceRulePage(param *form.InstanceRulePageReqParam) interface{} {
	...
	page := util.Paginate(param.PageSize, param.Current, "SELECT t2.id, t2.`name`, t2.metric_name AS monitorItem, t2.trigger_condition AS ruleCondition, product_type, monitor_type, t1.create_time  FROM   t_alarm_rule_resource_rel t1   JOIN t_alarm_rule t2 ON t2.id = t1.alarm_rule_id  WHERE t1.resource_id =?  AND t2.deleted = 0  ORDER BY create_time DESC, NAME ASC", sqlParam, &modelList)
