...
func (d *AlertContactGroupDao) SelectAlertContactGroup(db *gorm.DB, param form.AlertContactParam) *form.AlertContactFormPage {
	var modelList = &[]form.AlertContactGroupForm{}
	var total int64
...
func (d *AlertContactGroupDao) SelectAlertGroupContact(db *gorm.DB, param form.AlertContactParam) *form.AlertContactFormPage {
	var modelList = &[]form.AlertContactForm{}
	var total int64
	db.Model(&model.AlertContactGroupRel{}).Where("tenant_id = ? AND group_id = ?", param.TenantId, param.GroupId).Count(&total)
	var alertContactFormPage = &form.AlertContactFormPage{
		Records: modelList,
		Current: param.PageCurrent,
		Size:    param.PageSize,
...
func (d *AlertContactGroupDao) Insert(db *gorm.DB, entity *model.AlertContactGroup) {
	db.Create(entity)
}

func (d *AlertContactGroupDao) Update(db *gorm.DB, entity *model.AlertContactGroup) {
	db.Save(entity)
}

func (d *AlertContactGroupDao) Delete(db *gorm.DB, entity *model.AlertContactGroup) {
	db.Where("tenant_id = ? AND id = ?", entity.TenantId, entity.Id).Delete(model.AlertContactGroup{})
}
