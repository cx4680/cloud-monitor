...
func (dao *AlertRecordDao) InsertBatch(records []*models.AlertRecord) {
	...
	dao.db.Create(&records)
}

func (dao *AlertRecordDao) FindAlertRuleBindNum(ruleId, instanceId string) int {
	var num int
	dao.db.Raw("select count(1) from t_alarm_instance t LEFT JOIN t_alarm_rule t2 ON t2.id = t.alarm_rule_id where t.instance_id = ? and t.alarm_rule_id = ? and t2.enabled = 1 and t2.deleted = 0", instanceId, ruleId).Scan(&num)
	return num
}
...
func (dao *AlertRecordDao) FindContactInfoByGroupIds(groupIds []string) []dtos.ContactGroupInfo {
	...
	dao.db.Raw("SELECT g.id as groupId, g.name as groupName FROM alert_contact_group g WHERE id IN(?)", groupIds).Scan(&groupList)
	for _, group := range groupList {
		var contacts []dtos.UserContactInfo
		dao.db.Raw("select t2.`name` as contactName  ,t2.id as contactId,GROUP_CONCAT(CASE t3.type WHEN 1 THEN t3.no  END) as phone,GROUP_CONCAT(CASE t3.type WHEN 2 THEN t3.no  END) as mail from alert_contact_group_rel  t LEFT JOIN alert_contact t2 on t2.id = t.contact_id LEFT JOIN alert_contact_information t3 on ( t3.contact_id = t2.id and  t3.is_certify=1) where t.group_id=?  and t2.`status`=1   GROUP BY contactId  order by contactName", group.GroupId).Scan(&contacts)
...
func (dao *AlertRecordDao) FindFirstInstanceInfo(instanceId string) *models.AlarmInstance {
	...
	dao.db.Raw("select * from t_alarm_instance where instance_id=? limit 1", instanceId).Scan(&alarmInstance)
