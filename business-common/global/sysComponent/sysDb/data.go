...
func (c *CommonInitializerFetch) Fetch(db *gorm.DB) ([]interface{}, []string) {
	...
	db.Migrator().DropTable(&commonModels.MonitorItem{}, &commonModels.MonitorProduct{}, &commonModels.ConfigItem{})

	tables = append(tables, &commonModels.ConfigItem{}, &commonModels.MonitorProduct{})
	tables = append(tables, &commonModels.ConfigItem{}, &commonModels.MonitorItem{})

	sqls = append(sqls, "INSERT INTO `monitor_product` (`id`, `name`, `status`, `description`, `create_user`, `create_time`, `route`) VALUES ('1', '云服务器ECS', 1, 'ecs', NULL, NULL, '/productmonitoring/ecs');",
		...
		"INSERT INTO `monitor_item` (`id`, `product_id`, `name`, `metric_name`, `labels`, `metrics_linux`, `metrics_windows`, `statistics`, `unit`, `frequency`, `type`, `is_display`, `status`, `description`, `create_user`, `create_time`) VALUES ('1', '1', 'CPU使用率', 'ecs_cpu_usage', 'instance', '100 - (100 * (sum(irate(ecs_cpu_seconds_total{mode=\"idle\",$INSTANCE}[3m])) / sum(irate(ecs_cpu_seconds_total{$INSTANCE}[3m]))))', '', '', '%', null, '2', '1', '1', null, null, null);",
