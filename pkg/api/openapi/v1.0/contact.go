...
func (acl *ContactCtl) CreateContact(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, openapi2.NewRespError(getErrorCode(err), c))
		return
	} else {
		result := struct {
			RequestId string
			ContactId string
		}{RequestId: openapi2.GetRequestId(c), ContactId: request.ContactBizId}
...
func getErrorCode(err error) *openapi2.ErrorCode {
...
var ContactErrorMap = map[string]*openapi2.ErrorCode{
	"手机号格式错误":      openapi2.PhoneFormatError,
	"邮箱格式错误":       openapi2.MailFormatError,
	"手机号和邮箱必须填写一项": openapi2.InformationMissing,
	"联系人名字不能为空":    openapi2.MissingContactName,
	"联系人ID不能为空":    openapi2.MissingContactId,
	"联系组名字不能为空":    openapi2.MissingGroupName,
	"联系组ID不能为空":    openapi2.MissingGroupId,
	"联系人名字格式错误":    openapi2.ContactNameFormatError,
	"联系组名字格式错误":    openapi2.GroupNameFormatError,
	"请至少选择一位联系人":   openapi2.MissingContact,
	"该租户无此联系人":     openapi2.TenantHaveNotContact,
	"该租户无此联系组":     openapi2.TenantHaveNotGroup,
	"联系组名重复":       openapi2.GroupNameRepeat,
	"无效激活码":        openapi2.InvalidActivationCode,
	"每个联系人最多加入" + strconv.Itoa(constant.MaxContactGroup) + "个联系组": openapi2.ContactGroupNumberExceeded,
	"有联系人已加入" + strconv.Itoa(constant.MaxContactGroup) + "个联系组":   openapi2.ContactGroupNumberExceeded,
	"有联系组已有" + strconv.Itoa(constant.MaxContactNum) + "个联系人":      openapi2.GroupContactNumberExceeded,
	"联系人限制创建" + strconv.Itoa(constant.MaxContactNum) + "个":        openapi2.ContactNumberExceeded,
	"联系组限制创建" + strconv.Itoa(constant.MaxGroupNum) + "个":          openapi2.GroupNumberExceeded,
}
